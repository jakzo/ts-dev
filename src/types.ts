declare module "optionator" {
  ...
  interface Main {
    <O extends OptionsGeneric<S>, S extends string>(
      settings: Settings<O, S>
    ): Optionator<O, S>;
  ...
  type OptionsGeneric<S extends string> = OptionGeneric<S>[] & {
    0: OptionGeneric<S>;
  };
  ...
  type OptionsToParsed<O extends OptionsGeneric<string>> = UnboxIntersection<
    UnionToIntersection<
      BoxedTupleTypes<
        { [K in keyof O]: O[K] extends Option ? OptionToParsed<O[K]> : unknown }
      >
    >
  >;
  ...
  type BoxedTupleTypes<T> = { [P in keyof T]: [T[P]] }[Exclude<
    keyof T,
    keyof unknown[]
  >];
